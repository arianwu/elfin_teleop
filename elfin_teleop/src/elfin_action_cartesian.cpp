...
ElfinActionCartesianNode::ElfinActionCartesianNode(){
	...
	joint_cmd_sub = nh.subscribe(JOINT_CMD_TOPIC, 1, &ElfinActionCartesianNode::jointCmdCallback, this);
...
void ElfinActionCartesianNode::poseCmdCallback(const geometry_msgs::Pose& msg){
	...
	// Instead of using the previous command, use the actual position
	geometry_msgs::Pose initial_pose;
	calculateFKClient(current_joint_state, initial_pose);
	tf2::Vector3 previous_position(initial_pose.position.x, initial_pose.position.y, initial_pose.position.z);
	tf2::Quaternion previous_orientation(initial_pose.orientation.x, initial_pose.orientation.y, initial_pose.orientation.z, initial_pose.orientation.w);	
	...
	double time_step = total_time / (float)(total_points - 1);
		...
		if (solution.position.size() > 0){
			...
			if (secure) {
				...
				for(size_t i=0; i<ELFIN_DOF_JOINTS; i++){
					//printf("%2.3f  ", abs(solution.position[i] - previous_joints[i]));
					if (abs(solution.position[i] - previous_joints[i]) > joint_position_threshold[i]/(total_points-1))
						limit = true;
				}
				//printf("\n");
				...
				if (limit){
					...
					for (size_t i=0; i<ELFIN_DOF_JOINTS; i++) {
						...
						//printf("%3.4f \t\t %3.4f \n", solution.position[i], previous_joints[i]);
					}
					//printf("\n\n");
...
void ElfinActionCartesianNode::jointCmdCallback(const sensor_msgs::JointState& msg){
	std::vector<double> actual_position = current_joint_state.position;
	std::vector<double> desired_position = msg.position;

	double exec_time = exec_time;

	bool limit = false;
	for(size_t i=0; i<ELFIN_DOF_JOINTS; i++){
		if (abs(desired_position[i] - actual_position[i]) > joint_position_threshold[i])
			limit = true;
	}
	...
	if (limit) {
		// Calculate the times required to reach goal using max velocity
		for (size_t i=0; i<ELFIN_DOF_JOINTS; i++)
			times[i] = abs(desired_position[i] - actual_position[i])/(joint_velocity_limit[i] * velocity_scale);
	...
	trajectory_msgs::JointTrajectoryPoint point;
	point.time_from_start = ros::Duration(exec_time);
	point.positions = desired_position;

	goal.trajectory.points.resize(1);
	goal.trajectory.points[0] = point;
